...
namespace HanZiToPinYin
{
    class Program
    {
        private static string conStr = "Data Source=db.frllk.com;Initial Catalog=***;Persist Security Info=True;User ID=sa;Password=***";
        /// <summary>
        /// 打开数据库链接方法
        /// </summary>
        ...
        {
            using (SqlConnection conn = GetSqlConnection(conStr))
            {
                var list = conn.Query<dynamic>("SELECT Id, Name FROM dbo.CookMaterial");
                foreach (var item in list)
                {
                    dynamic id = item.Id;
                    dynamic name = item.Name;
                    var count = conn.Execute(
                         $"UPDATE dbo.CookMaterial SET PinYin = '{Pinyin.ToPinYin(name)}',PY= '{Pinyin.ToPy(name)}' WHERE Id ={id}");
                    if (count <= 0) Console.WriteLine($"Id {id} not successly insert .");
                }
            }
            Console.WriteLine("successly !");
            Console.ReadKey();
        }
    ...
    {
        private static readonly int[] MSpellValue =
        {
        ...
        public static string Convert(string str, bool first = false)
        {
            ...
            foreach (char t in chars)
            {
                ...
                if (array.Length > 2 || array.Length <= 0) break;
                if (array.Length == 1) sb.Append(t.ToString().ToLower());
                else
                {
                    int i1 = array[0];
                    int i2 = array[1];
                    var chrasc = i1 * 256 + i2 - 65536;
                    if (chrasc > 0 && chrasc < 160) sb.Append(t);
                    else
                    {
                        for (int i = (MSpellValue.Length - 1); i >= 0; i--)
                        {
                            if (MSpellValue[i] <= chrasc)
                            {
                                sb.Append(first ? MSpellString[i].Substring(0, 1) : MSpellString[i]);
                                break;
                            }
                        }
                    }
        ...
        public static string ToPy(string hanzi)
        {
            return Convert(hanzi, true).ToUpper();
        }
